...
然后客户端通过socket和connect两个函数来主动打开连接，给服务器发送带有syn标志的分组，随机生成一个初始序列号x，以及附带MSS（最大段的大小）等额外信息，为了避免在网络层被ip协议分片使得使得丢失错误的概率增加，以及为了达到最佳的传输效果，mss的值一般为 以太网MTU（最大传输单元）的值减去ip头部和tcp头部大小，等于1460字节
