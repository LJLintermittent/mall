...
~~~wiki
一致性哈希的应用的基本场景，假设我有N台缓存服务器做一个分布式缓存，那么数据进来后需要放到某一台机器上做一个分布式存储，那么为了让数据均匀的散布到这些服务器，我可以使用对数据进行hash然后取模运算N，就会把这一条数据放到某一个机器节点中，但是对于分布式系统来说，如果一个服务器crash了，或者新加进来一个服务器，那么这个映射公式就会发生改变。映射公式发生了改变，意味着所有的缓存都会失效，因为取的时候也是通过这种映射关系来取的，那么大面积缓存失效的话请求都跑到数据库中，非常危险。
那么一致性哈希算法就是解决在添加或删除一个节点的时候，尽可能小的改变已有key的映射关系
一致性哈希的做法就是使用一个相同的哈希函数将对象（也就是要存储进来的数据，或者查找的数据的key）和机器节点映射到一个哈希空间中，这个空间可以是一个虚拟的环状结构，接下来考虑的就是将对象映射到节点中，那么做法就是从对象的哈希地址开始触发，顺时针寻找第一个节点，那么就映射到这个节点上，由于是一个对象的hash和节点的hash都是固定的，在一个环上，所以对象到节点之间的映射关系是唯一的
同样的，如果某一个节点宕机了，它的影响也只是从他这个位置开始到上一个节点位置之间的对象，这些对象需要重新映射。
另外虚拟节点平衡性问题就不说了
~~~
...
如果我们设置的线程数过小的话，如果同一时间有大量请求过来，可能会导致大量任务堆积在等待队列中，或者很快撑满队列导致最大线程数启动，然后也被撑满，最后会拒绝掉很多的任务，导致cpu的利用不充分，失去了多核cpu使用多线程的意义

但是如果线程数量设置过大，大量线程在争夺cpu，这样会导致大量的上下文切换，从而增加单个线程的执行时间，异步的效果没有得到体现

所以针对线程数量的设置，有两种参考方案：

1.cpu密集型，简单理解就是使用cpu做大量计算，比如对内存中的数据进行排序等

2.io密集型，涉及到网络的读取，文件读取都是io密集型

针对cpu密集型，可以将线程数设置为N+1（N为cpu的核心数），比核心多一个的线程是为了其他原因导致任务中断的 情况下，一旦任务暂停，cpu就会处于空闲状态，而这种状态多出来一个线程就会充分利用cpu

针对io密集型，使用N*2，这种应用大部分时间来处理io交互，而线程在处理io交互的时候是不会占用cpu来处理的，所以可以分配多一点线程